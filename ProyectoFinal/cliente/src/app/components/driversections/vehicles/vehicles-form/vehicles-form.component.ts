...
export class VehiclesFormComponent implements OnInit {
  ...
  async signup() {
    ...
    for (var y = 0; y < driversChecked.length; y++) {
      for (let x = 0; x < this.drivers.length; x++) {
        if (driversChecked[y].id === this.drivers[x]._id) {
    ...
    if (action === 'create') {
      await this.vehicleService.createVehicule(data, this.token);
      this.refresh();
      alert('create');
    }
    if (action === 'edit') {
      ...
      await this.vehicleService.editVehicule(newdata, this.token);
      this.refresh();
    }
  }
  async driver() {}
